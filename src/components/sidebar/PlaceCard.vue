...
const isLiked = computed(() => likeStore.isLiked(Number(props.place.id)));
...
watch(() => props.place, checkOverflow, { deep: true });

const categoryCssVarName = computed(() => getCategoryVarName(props.place.categoryCode));
const tagInlineStyle = computed(() => ({
  '--tag-bg': `var(--category-tag-bg-${categoryCssVarName.value})`,
}));

const isDraggable = computed(() => route.path.startsWith('/plan/'));
